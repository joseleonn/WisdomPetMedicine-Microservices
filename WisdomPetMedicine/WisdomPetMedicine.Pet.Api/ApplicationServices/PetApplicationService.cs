...
namespace WisdomPetMedicine.Pet.Api.ApplicationServices
{
    public class PetApplicationService
    {
                                     ...
                                     ILogger<PetApplicationService> logger)
        {
            ...
            RegisterEventHandlers();//este metodo sirve para configurar Azure Bus Service y que es mapee con el topic correspondiente cada vez que se instancie la clase
        }
        ...
        //configurar Azure Bus Service al topic
        ...
        //enviar mensajes de integracion a Azure Service Bus
        ...
        private async Task PublishIntegrationEventAsync(IIntegrationEvent integrationEvent, string connectionString, string topicName)
        {
            ...
            try
            {
                ...
                logger.LogInformation($"Integration event sent successfully: {integrationEvent.GetType().Name}");
            }
            catch (Exception ex)
            {
                logger.LogError($"Failed to send integration event: {integrationEvent.GetType().Name}. Error: {ex.Message}");
                throw;
            }
        }
    }
